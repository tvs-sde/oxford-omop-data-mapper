...
- **XML structure** (NOTE: SQL should be directly in `<Sql>` tags, NO CDATA wrapper, NO comments):
```xml
<Query>
    <Sql>
with LU as (
...
**CRITICAL**: The XML file will contain pure SQL without:
- ❌ CDATA wrappers (`<![CDATA[` and `]]>`)
- ❌ SQL comments (`-- comment text`)
- ✅ Just clean, executable SQL directly in `<Sql>` tags
...
**CRITICAL RULE #1: NEVER MODIFY EXISTING TRANSFORMATION TOOLS**

⛔ **NEVER edit, modify, or adapt existing selectors, lookups, or resolvers** (e.g., `Icdo3Selector`, `Opcs4Selector`, `TumourLateralityLookup`)

✅ **If a transformation tool doesn't match your needs:**
- **Create a NEW selector/lookup/resolver class** with the correct signature
- **Example**: If `Icdo3Selector` requires 2 inputs but you only have 1:
  - ❌ DO NOT modify `Icdo3Selector` to accept 1 input
  - ✅ CREATE `Icdo3MorphologyOnlySelector` that accepts 1 input
- **Example**: If `TCategoryLookup` doesn't handle a specific field variation:
  - ❌ DO NOT modify `TCategoryLookup`
  - ✅ CREATE `TCategoryVariantLookup` with the needed logic

**Rationale**: Existing tools are used by many transformations. Modifying them breaks existing code.

**CRITICAL RULE #2: LOOKUP DETECTION**

Before applying any transformation attribute, ALWAYS check if an NHS Data Dictionary lookup exists for the field name. If a matching lookup class is found, use it instead of generic concept selectors.
